...
Due to the aforementioned Planck lengths, the rope must be quite short; in fact, the head (_H_) and tail (_T_) must **always be touching** (diagonally adjacent and even overlapping both count as touching):
...
Using the same series of motions as the above example, but with the knots marked _H_, _1_, _2_, ..., _9_, the motions now occur as follows:
...
Now, the tail (_9_) visits _**36**_ positions (including _s_) at least once:
